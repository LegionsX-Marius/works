...
function setupDashboard(){
  ...
  } else if(ses.grade === 'Sef Spital' || ses.grade === 'Responsabil Activitate'){
    reportAll.classList.remove('hidden');
    renderWeeklyAll();
    reportSelf.classList.remove('hidden');
    renderSelfPanels(ses);
...
// ===== Login logic =====
function setupLogin(){
  // ascunde selectorul de grad (nu se mai folosește)
  const gradeSelect = $('#l_grade');
  if(gradeSelect){ gradeSelect.parentElement.style.display = 'none'; }

  // PIN field optional (existent doar pentru Director / Vice-Director)
  let pinField = document.querySelector('#l_pin');
  if(!pinField){
    const pinLabel = document.createElement('label');
    pinLabel.textContent = 'PIN (doar pentru Director/Vice-Director)';
    pinField = document.createElement('input');
    pinField.id = 'l_pin';
    pinField.type = 'password';
    pinField.placeholder = 'ex: 1234';
    const turaEl = $('#l_tura');
    turaEl.parentElement.insertBefore(pinLabel, turaEl);
    turaEl.parentElement.insertBefore(pinField, turaEl);
  }
  ...
  $('#loginBtn').addEventListener('click', () => {
    ...
    if(!name || !id){ alert('Completează Nume și ID.'); return; }
    const person = getPersons().find(p => p.id===id && p.name.toLowerCase()===name.toLowerCase());
    if(!person){ alert('Utilizatorul nu există. Solicită adăugare la Director/Vice-Director.'); return; }

    // Protecție pentru Director / Vice-Director: necesită PIN setat la creare
    if(person.grade === 'Director' || person.grade === 'Vice-Director'){
      const pin = (pinField.value || '').trim();
      if(!person.pin){
        alert('Acest profil necesită PIN. Cere Directorului să seteze PIN la adăugare.');
        return;
      }
      if(pin !== String(person.pin)){
        alert('PIN incorect pentru Director/Vice-Director.');
        return;
      }
    }
...
// ===== Add default Director (Marius) =====
(function(){
  let persons = getPersons();
  if (!persons.some(p => p.id === 138)) {
    persons.push({
      id: 138,
      name: "Marius",
      role: "Director",
    ...
    savePersons(persons);
    console.log("Default Director Marius added.");
  }
})();
